...
static int imx585_set_gain(struct imx585 *imx585, u32 value)
{
	int ret = 0;
	u8 val;
	pr_err("%slxp4  gain value 0x%x", __func__, value);
	...
	pr_err("%s lxp5 read gain value 0x%x", __func__, val);
	imx585_read_reg(imx585, IMX585_GAIN+1, &val);
	pr_err("%s lxp6 read gain value 0x%x", __func__, val);
	return ret;
}
...
static int imx585_set_exposure(struct imx585 *imx585, u32 value)
{
	int ret = 0;
	u8 val;
	pr_err("%s lxp1 expor value 0x%x", __func__, value);
	...
	pr_err("%s lxp2 read exp value 0x%x", __func__, val);
	val = 0;
	imx585_read_reg(imx585, IMX585_EXPOSURE+1, &val);
	pr_err("%s lxp3 read exp value 0x%x", __func__, val);
	return ret;
}
